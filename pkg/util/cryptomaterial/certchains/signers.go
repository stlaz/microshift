...
type CertificateRequestInfo interface {
	GetMeta() CSRMeta
}
...
func (s *CertificateSigner) Regenerate(certPath ...string) error {
	...
	case 1: // renew a direct sub-cert
		if err := s.regenerateSubCA(certPath[0]); err != nil && !IsSignerNotFoundError(err) {
			return err
		}
...
func WhenToRotateAtEarliest(cs *CertificateChains) ([]string, time.Time, error) {
	...
	err := cs.WalkChains(nil, func(currentPath []string, c x509.Certificate) error {
		...
		rotateAt := c.NotBefore.Add(time.Duration(totalTime * rotateAtLifetime))
